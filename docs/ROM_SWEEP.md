...
特定の機種やクロック速度でしか動作しないタイトルに対応するため、以下の順序で
自動的に再試行を行う。途中で `ok` 判定（30s/60s 両方が ok）になれば、その時点で
確定し、以降の構成は試行しない。

1. **V2-4MHz** (デフォルト): `CLOCK_4MHZ=1`, `BOOTTEST_DIPSW2` 未指定 (0x71)
2. **V1H-4MHz**: `CLOCK_4MHZ=1`, `BOOTTEST_DIPSW2=0xF1` (V1 ハイスピード)
3. **V2-8MHz**: `CLOCK_4MHZ` 未指定, `BOOTTEST_DIPSW2` 未指定 (0x71)

`report.json` の `"config"` フィールドに、最終的にどの構成が採用されたかが
記録される。
...
2026-05-09 `--all`: 1132本中 300本 NG。
再試行ロジック（V1H, 8MHz）と判定ロジックの改善（nonblack_px 閾値導入）により、
以下の効果を確認：
- **誤検出防止**: 「ジェムストンウォーリア」等、Text VRAM に BASIC 残骸があるが
  GVRAM に描画があるタイトルを正しく `ok` 判定。
- **V1H 救済**: 「風の谷のナウシカADV」「道鏡」等が V1H モードで起動成功。
- **8MHz 救済**: 「ドラゴンナイト２」「ソー・アンド・ソー」等が 8MHz モードで起動成功。
現状の互換性は約 73.5% (832/1132)。
