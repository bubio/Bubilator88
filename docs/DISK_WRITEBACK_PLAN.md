...
| キー | 型 | デフォルト | 説明 |
|------|-----|----------|------|
| `diskWriteBackEnabled` | Bool | true | ディスクへの書き戻し全体を有効化 |
| `diskWriteBackMode` | String | "auto" | `auto` (黙って保存) / `confirm` (eject 時に確認ダイアログ) / `manual` (明示メニュー操作時のみ) |

初期実装では `auto` のみで動かし、`confirm` / `manual` は Phase 2 以降。

## 4. UI 変更

### 4.1 設定画面 (SettingsView)
- 「ディスク」セクションを新設
  - トグル: 「ディスクへの書き戻しを有効にする」
  - セグメント: 自動 / 確認 / 手動

### 4.2 メニュー (Bubilator88App)
- File メニュー (または Disk メニュー) に以下を追加 (`manual` モード時に
  必要だが `auto` でも便利):
  - 「ドライブ 0 のディスクを保存」
  - 「ドライブ 1 のディスクを保存」

### 4.3 ステータスバー / トースト
- 書き戻し発生時に「ディスクを保存しました (ドライブ N)」をトースト表示
- 書き戻し失敗 / リカバリディレクトリへ退避時はアラート
...
### Phase 3: UI / 設定
1. Settings に `diskWriteBackEnabled` / `diskWriteBackMode` 追加
2. SettingsView にトグル + セグメント追加
3. SettingsView に「ディスクキャッシュ管理」セクション
   - 使用量表示 / 個別削除 / 全削除
   - キャッシュごとに元アーカイブパスとエントリ一覧を表示
4. File メニューに「ドライブ N のディスクを保存」追加
5. トースト/アラート整備
6. 元アーカイブが見つからないキャッシュへの注記表示
