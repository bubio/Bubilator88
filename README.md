...
Bubilator88 は、NEC パーソナルコンピュータ PC-8801mkIISR以降 の macOS ネイティブエミュレーターです。

**このプロジェクトのコードは、ほぼすべて AI（Claude, Codex）によって書かれています。**

レトロ PC エミュレーターは、未文書化されたハードウェアの挙動再現、T ステート精度のタイミング制御、複数 LSI の協調動作など、深い専門知識と緻密な実装を要求される特殊なソフトウェアです。「AI はこの種のソフトウェアをゼロから構築できるのか？」— Bubilator88 はその問いに対する実験であり、挑戦です。
...
- **AI Upscale (Balanced)** — SRVGGNet (64ch × 16層、約 60万パラメータ)。PC-8801 画面に特化した知識蒸留モデルで、Real-ESRGAN の出力を正解データとして約 4,700 枚のゲーム画面スクリーンショットで学習。画質と速度のバランスが取れた標準モードです。
...
エミュレータの画面テキストを macOS のクリップボードへコピー (Cmd+Shift+C)、またはクリップボードのテキストを擬似キー入力として流し込む (Cmd+V) ことができます。半角カナを含む BASIC プログラムのリスト転記や、サンプルコードの貼り付けが手打ちなしで行えます。ペースト中は ESC キーでキャンセル可能です。
